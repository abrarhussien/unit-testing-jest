import {sum, isPalindrome, letterNumber, area, Reverse,startsWith, Second_Greatest_Lowest, abc, substrings, shuffle, makeid} from "./index"
...
test("startsWith return elements start with a letter  ",()=>{
    expect(startsWith(["abrar","mmmm","aaaaa","any","babb"],"a")).toEqual(["abrar","aaaaa","any"]);
})
...
test("shuffle shuffles an array ",()=>{
    const mathSpy = jest.spyOn(Math, 'random').mockReturnValueOnce(2/3)
    .mockReturnValueOnce(2/2).mockReturnValueOnce(1/1)
...
test("makeid make an uniqe id",()=>{
    const mathSpy = jest.spyOn(Math, 'random').mockReturnValueOnce(1/62)
    .mockReturnValueOnce(1/62).mockReturnValueOnce(2/62)
